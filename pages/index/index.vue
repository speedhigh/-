<template>
	<view class="relative text-sm text-gray-800 bg-gray-200">
		...
		<view class="relative">
			...
			<!-- 分类筛选区域 -->
			<view class="sticky bg-gray-200 top-0" style="z-index: 999">
	...
	export default {
		...
		methods: {
			...
			async getPromotionProduct() {
				...
				this.time = (Date.parse(res.data.data[0].dendtime) - Date.parse(new Date()))
			...
			async getTabTitleList() {
				...
				this.currentPage = 1
				let obj = { countryCode: res.data.data[0].scountry, onefunctioncategory: res.data.data[0].sfuncid }
				Object.assign(this.params, obj)
			},
			changeTabs(countryCode, secondClass) {
				this.currentPage = 1
				let obj = { countryCode: countryCode, onefunctioncategory: secondClass }
				Object.assign(this.params, obj)
				uni.pageScrollTo({ scrollTop: 1145, duration: 100 })
			},
			// 加入购物车
