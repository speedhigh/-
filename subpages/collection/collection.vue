		...
		<view v-show="!showLoading && list.length > 0"  style="padding: 10rpx 20rpx">
			<view class="flex flex-wrap justify-between">
				<view v-for="(item, index) in list" :key="index">
					<navigator :url="'/pages/detail/detail?id=' + item.product.id">
						<muzi-card :item="item.product" />
					</navigator>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			async getList() {
				const res = await this.$api({ url: '/myfavorite/getListCollect', data: {userid: uni.getStorageSync('user').id} })
